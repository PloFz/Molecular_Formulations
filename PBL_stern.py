...
def PBL_stern(mol_name, mesh_density, ep_in=4., ep_ex=80., kappa=0.125):
    ...
    rhs = np.concatenate([charged_grid_fun.coefficients,
			  charged_grid_dfun.coefficients,
			  np.zeros(n_elmts_ex),
			  np.zeros(n_elmts_ex)])
    ...
    ident_in  = sparse.identity(dirichl_space_in, dirichl_space_in, dirichl_space_in)
    ...
    # modified version: d/dn {(1)+(2)}
    ident_din = sparse.identity(neumann_space_in, neumann_space_in, neumann_space_in)
    ...
    ident_ex = sparse.identity(dirichl_space_ex, dirichl_space_ex, dirichl_space_ex)
    # Internal Boudary
    ...
    #blocked[1, 1] = (ep_in/ep_ex)*slp_in   # slp_in = slp_1T1
    #blocked[1, 2] =  dlp_2T1
    #blocked[1, 3] = -slp_2T1

    # +(1)
    #blocked[1, 0] = ident_in
    #blocked[1, 1] = (ep_in/ep_ex - 1)*slp_in
    #blocked[1, 2] =  dlp_2T1
    #blocked[1, 2] = -slp_2T1

    # +(1) & derivated formulation
    #blocked[1, 0] =  0
    blocked[1, 1] =  ident_din + (ep_in/ep_ex - 1)*adj_1T1
    blocked[1, 2] = -hyp_2T1
    blocked[1, 3] = -adj_2T1

    blocked[2, 0] = -dlp_1T2
    blocked[2, 1] = (ep_in/ep_ex)*slp_1T2
    ...
    x, info = gmres(op_discrete, rhs, callback=iteration_counter, tol=1e-3, maxiter=1000, restart = 1000)
