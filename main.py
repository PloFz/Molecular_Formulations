...
mol_name = '7ry'
red = np.array([ .1, .2, .4, .6, .8, 1., 1.2, 1.5, 2., 3., 4. ])

for i in range(len(red)):
# #	PBL.solvation_energy(mol_name, densities[i], info=True)
# #   	PBL.solvation_energy(mol_name, densities[i], formulation='juffer', info=True)
 	PBL_stern.solvation_energy(mol_name, 3., stern_radius=red[i], info=True)
 	gc.collect()
